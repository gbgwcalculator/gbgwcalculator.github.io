...
const exportOrder = [ 'events', 'series', 'skills', 'pilots', 'units', 'parts' ];

const stores = {
  events: {
    filename: `../../src/data/csv/events.csv`
  },
  parts: {
  ...
  series: {
    filename: `../../src/data/csv/series.csv`
  },
  skills: {
...
const tables = {
  ...
  pilots: {
    ...
    columns: [
      ...
      {name: 'obtained_other', field: 'Other'},
  ...
  units: {
    ...
    columns: [
      ...
      {name: 'obtained_capsule', field: 'Capsule'},
      {name: 'obtained_exchange', field: 'Exchange'},
      {name: 'obtained_mission', field: 'Mission'},
      {name: 'obtained_other', field: 'Other'},
      {name: 'limited', field: 'Limited', type: 'boolean'},
      {name: 'release_date', field: 'Release Date'},
      {name: 'notes', field: 'Notes'}
    ]
  },
...
const onReady = () => {
  ...
  document.querySelector('.btn-download').addEventListener('click', handleDownloadAll);
}

const handleDownloadAll = (e) => {
  const statements = [ 'USE `gbgw`;' ].concat(exportOrder.map(id => document.querySelector('#' + id).value));
  saveToDisk(statements.join('\n\n'), 'data-only.sql');
};
